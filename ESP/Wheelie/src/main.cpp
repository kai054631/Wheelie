...
void TaskMonitorCode(void *pvParameters) {
  for (;;) {
    printf("Pitch:%f,Gyro:%f,Out:%f\n", Pitch_angle, Pitch_gyro, output_voltage);
...
void setup() {
  ...
  encoderL.init(); encoderL.enableInterrupts(doLA, doLB);
  motorL.linkSensor(&encoderL);
  driverL.voltage_power_supply = 12; driverL.init();
  motorL.linkDriver(&driverL);
  motorL.controller = MotionControlType::torque;
  motorL.init(); motorL.initFOC();

  encoderR.init(); encoderR.enableInterrupts(doRA, doRB);
  motorR.linkSensor(&encoderR);
  driverR.voltage_power_supply = 12; driverR.init();
